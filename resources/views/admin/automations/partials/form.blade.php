        ...
        @if($isEdit)
            <select name="page_id" class="form-select" required>
                ...
                @foreach($pages as $page)
                    <option value="{{ $page->id }}" @selected((int) old('page_id', $automation->page_id) === $page->id)>{{ $page->page_name }} ({{ $page->page_id }})</option>
                @endforeach
            </select>
        @else
            <select name="page_ids[]" class="form-select" multiple required>
                @foreach($pages as $page)
                    <option value="{{ $page->id }}" @selected(in_array($page->id, old('page_ids', [])))>{{ $page->page_name }} ({{ $page->page_id }})</option>
                @endforeach
            </select>
            <small class="text-muted">Select multiple pages to create one automation config per page.</small>
        @endif
    </div>
</div>
